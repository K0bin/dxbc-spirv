  ...
  for (const auto& operand : layout.operands) {
    ...
    switch (operand.kind) {
      ...
      case OperandKind::ePred:
        if ((inBounds = nPred++ == 0u && op.hasPred()))
          disassembleOperand(stream, op, op.getPred());
      break;
...
void Disassembler::disassembleOperand(std::ostream& stream, const Instruction& op, const Operand& arg) const {
  if (op.getOpCode() == OpCode::eDcl) {
    ...
    disassembleRegisterAddressing(stream, op, arg);
    return;
  }
  ...
  disassembleRegisterType(stream, arg.getRegisterType());
  disassembleRegisterAddressing(stream, op, arg);
  if (arg.getComponentCount(m_info) == ComponentCount::e4Component) {
  ...
  if (modifier == OperandModifier::eDz || modifier == OperandModifier::eDw) {
    stream << " / ";
    disassembleRegisterType(stream, arg.getRegisterType());
    disassembleRegisterAddressing(stream, op, arg);
    if (arg.getComponentCount(m_info) == ComponentCount::e4Component) {
...
void Disassembler::disassembleRegisterAddressing(std::ostream& stream, const Instruction& op, const Operand& arg) const {
  if (arg.getRegisterType() == RegisterType::eMiscType) {
    ...
    if (arg.hasRelativeAddressing()) {
      ...
      disassembleOperand(stream, op, op.getRawOperand(arg.getRelAddrOperand()));
      stream << "]";
    } else {
      stream << arg.getIndex();
    }
