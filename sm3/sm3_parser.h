...
public:
  ...
  int8_t getShift() const {
    return m_info.kind == OperandKind::eDstReg
             ? util::bextractSigned<int8_t, uint32_t>(m_token, 24u, 4u)
             : 0;
  }
  ...
  OperandModifier getModifier() const {
    return m_info.kind == OperandKind::eSrcReg || m_info.kind == OperandKind::ePred
             ? OperandModifier(util::bextract(m_token, 24u, 4u))
             : OperandModifier::eNone;
  }

  /** Queries the semantic usage.
   *  Only useful for declaration operands of shader inputs and outputs. */
  Usage getUsage() const {
    return m_info.kind == OperandKind::eDcl
            ? Usage(util::bextract(m_token, 0u, 4u))
            : Usage::eTexCoord;
  }
  ...
  TextureType getTextureType() const {
    return m_info.kind == OperandKind::eDcl
              ? TextureType(util::bextract(m_token, 27u, 4u))
              : TextureType::eTexture2D;
  }
...
public:
  ...
  const Operand& getImm() const {
    dxbc_spv_assert(m_immOperand.has_value());
    return getRawOperand(m_immOperand.value());
  }
...
private:
  ...
  std::optional<uint8_t> m_immOperand  = { };
